    ...
    async def initialize_debate(self):
        ...
        for idx, item in enumerate(results):
            ...
            if isinstance(item, Exception):
                ...
                print(f"⚠️ Exception for {debater}: {str(item)}")
                result = TaskResult(
                    provider="error",
...
Game theory position (cooperate|defect|punish|reward):"""
        ...
        if any(eval.veto_called for eval in self.judge_evaluations[-len(self.JUDGES):]):
            ...
            print(f"⚠️ Judge veto called in round {round_num} - stopping debate early")
    ...
    async def _execute_judge_evaluation(self, round_num: int):
        ...
        print(f"⚖️ BALANCED HIGH CONCURRENCY: {len(self.JUDGES)} judges firing with 6 simultaneous calls...")
        ...
        for idx, item in enumerate(results):
            if isinstance(item, Exception):
                ...
                print(f"⚠️ Exception for judge {judge}: {str(item)}")
                result = TaskResult(
                    provider="error",
        ...
        async def fire_closing(debater: str):
            ...
            async with semaphore:
                try:
                    ...
                    return debater, TaskResult(provider='error', response=f"Error: {str(e)}", latency=0.0, tokens=0, cost=0.0)

        tasks = [fire_closing(d) for d in self.DEBATERS]
        results = await asyncio.gather(*tasks, return_exceptions=True)

        for idx, item in enumerate(results):
            if isinstance(item, Exception):
                debater = self.DEBATERS[idx]
                result = TaskResult('error', f"Exception: {str(item)}", 0.0, 0, 0.0)
            else:
                try:
                    debater, result = item
                except Exception:
                    debater = self.DEBATERS[idx]
                    result = TaskResult('error', 'Malformed result', 0.0, 0, 0.0)
    ...
    async def execute_judges_debate(self):
        """Have the judges debate/synthesize among themselves to create a final judge-only verdict."""
        print("\n⚖️ EXECUTING JUDGES-ONLY DEBATE / SYNTHESIS")
        prompt = f"Judges: debate the final positions and synthesize a short verdict covering legality (constitutional compliance), technical feasibility, and prioritized actionable steps for Phase 2." 
        semaphore = asyncio.Semaphore(6)

        async def fire_judge_synthesis(judge: str):
            async with semaphore:
                try:
                    provider = judge.split('-')[0] if judge != 'together-ai' else 'openai'
                    return judge, await asyncio.wait_for(router.route_task(prompt, provider, max_tokens=1500), timeout=180.0)
                except Exception as e:
                    return judge, TaskResult(provider='error', response=f"Error: {str(e)}", latency=0.0, tokens=0, cost=0.0)

        tasks = [fire_judge_synthesis(j) for j in self.JUDGES]
        results = await asyncio.gather(*tasks, return_exceptions=True)

        # Store judge syntheses as pseudo-turns
        for idx, item in enumerate(results):
            if isinstance(item, Exception):
    ...
    def _extract_consensus(self, results) -> List[str]:
        """Extract consensus points from judge syntheses"""
        ...
        return consensus[:10]  # Top 10 points

async def run_phase1_debate(mission_file=None):
    """Main entry point for Phase 1 debate execution"""
    print("🌀 AXIOM-X PHASE 1: ADVERSARIAL DEBATE INITIALIZATION")
    print("=" * 70)

    # Read mission if provided
    mission = ""
    if mission_file:
        try:
            with open(mission_file, 'r', encoding='utf-8') as f:
                mission = f.read()
            print(f"✅ Loaded mission from {mission_file} ({len(mission)} chars)")
        except Exception as e:
            print(f"⚠️ Could not load mission file {mission_file}: {e}")

    # Initialize orchestrator
    orchestrator = Phase1Orchestrator(mission=mission)

    # Parse CLI args (allow forced early closure)
    parser = argparse.ArgumentParser()
    parser.add_argument('--force-close-after', type=int, default=None,
                        help='If set, once debate_log reaches this many turns perform closing and judge synthesis (e.g. 18).')
    parser.add_argument('--max-rounds', type=int, default=20, help='Maximum debate rounds to run')
    args, _ = parser.parse_known_args()

    try:
        # Initialize debate
        await orchestrator.initialize_debate()

        # Start status update task after start_time is set
        async def status_updates():
            """Write status updates every 30 seconds to stdout and a heartbeat file"""
            hb_path = orchestrator.output_dir / 'heartbeat.json'
            while True:
                await asyncio.sleep(30)
                if orchestrator.start_time is None:
                    continue
                elapsed = time.time() - orchestrator.start_time
                resonance = orchestrator.chaos_tracker.calculate_resonance()
                status = {
                    'timestamp': datetime.now().isoformat(),
                    'elapsed_seconds': elapsed,
                    'turns': len(orchestrator.debate_log),
                    'resonance': resonance,
                    'turn_counts': orchestrator.turn_counts
                }
                try:
                    ...
                    pass
                print(f"\n📊 STATUS UPDATE ({elapsed/60:.1f}min): Turns={len(orchestrator.debate_log)}, Resonance={resonance:.3f}")
                print(f"   Turn counts: {dict(orchestrator.turn_counts)}")
        ...
        round_num = 1
        while not orchestrator.check_convergence():
            await orchestrator.execute_debate_round(round_num)
            round_num += 1

            # If user asked to force-close after N turns, trigger closing+judges
            if args.force_close_after is not None and len(orchestrator.debate_log) >= args.force_close_after:
                print(f"\n✂️ Force-closing debate after {len(orchestrator.debate_log)} turns as requested.")
                # Round 19: closing arguments
                await orchestrator.execute_closing_arguments()
                # Round 20: judges debate/synthesize
                await orchestrator.execute_judges_debate()
                break
        ...
        print(f"   - Total turns: {len(orchestrator.debate_log)}")
        print(f"   - Resonance achieved: {orchestrator.chaos_tracker.calculate_resonance():.3f}")
        print(f"   - Judge evaluations: {len(orchestrator.judge_evaluations)}")
...
if __name__ == "__main__":
    # Allow passing CLI args to run_phase1_debate via argparse inside
    asyncio.run(run_phase1_debate())